...
- [x] (Jules) **Async Pipeline Refactor:** Convert seed script to use asyncio/Semaphore and generate 10,000 JSONL rows (P2-T11)
...
- [x] **Summary-Based Complexity Scoring:** Update the ML dispatcher to embed `[Chat Summary] + [Final Prompt]` rather than just the final prompt, giving the ML classifier true context-awareness (P3-T14)
- [x] (Jules) **Dispatcher Architecture Refactor:** Modularize `dispatcher.py` and fix the streaming usage tracking bug and hardcoded cost maps (P3-T15)
