...
class RouterDispatcher:
    ...
    def _enforce_budget_limits(self) -> bool:
        budget_limit = self.settings.router.budget_limit_usd
        return budget_limit > 0 and usage_tracker.total_spent_usd >= budget_limit

    def _determine_initial_tier(
        self, request: ChatCompletionRequest, is_budget_exceeded: bool
    ) -> tuple[str, float]:
        ...
        # 1. Score the prompt (using the last user message + optional summary)
        prompt_text = ""
        summary_text = ""
        for message in request.messages:
            if message.content.startswith("[Chat Summary]"):
        ...
        full_prompt = f"{summary_text}\n{prompt_text}" if summary_text else prompt_text
        score = self.classifier.score_prompt(full_prompt.strip())
    ...
    async def _enforce_context_capacity(
        self, request: ChatCompletionRequest, tier_name: str, is_budget_exceeded: bool
    ) -> str:
        # Resolve summarizer API key
        ...
        # Context-Aware Capacity Routing: cascade tiers, compress first before upgrading
    ...
    async def _execute_with_fallback(
        self, request: ChatCompletionRequest, tier_name: str, score: float
            ...
            try:
                ...
                if request.stream:
                    ...
                    async def _stream_with_usage_tracking(
                        gen: AsyncGenerator[str], actual: float, hypothetical: float
                    ) -> AsyncGenerator[str]:
                        try:
                            async for chunk in gen:
                                yield chunk
                        finally:
                            usage_tracker.record_usage(actual, hypothetical)

                    response_gen = client.stream_generate(request)
                    actual_cost = getattr(current_config, "cost_per_1k_tokens", 0.0)
                    hypothetical_cost = getattr(self.settings.tiers.smart, "cost_per_1k_tokens", 0.0)
                    return _stream_with_usage_tracking(response_gen, actual_cost, hypothetical_cost), current_config.model, score
                else:
    ...
    async def dispatch(
        ...
        is_budget_exceeded = self._enforce_budget_limits()

        tier_name, score = self._determine_initial_tier(request, is_budget_exceeded)

        if request.routing_preference not in (RoutingPreference.frontier_only, RoutingPreference.economy):
            tier_name = await self._enforce_context_capacity(request, tier_name, is_budget_exceeded)

            is_shadow = shadow_mode or self.settings.router.shadow_mode
            if is_shadow:
                logger.info(
        ...
        return await self._execute_with_fallback(request, tier_name, score)
